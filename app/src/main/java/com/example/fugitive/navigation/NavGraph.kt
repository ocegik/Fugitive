...
fun AppNavGraph(navController: NavHostController) {
    // ✅ Source of truth is authPreferences (saved session), NOT Firebase.currentUser
    // Firebase.currentUser persists even after a failed/partial sign-up, causing ghost sessions.
    val authPreferences: AuthPreferences = koinInject()
    val startDestination = if (authPreferences.getUserId() != null) Screen.Home.route else Screen.Welcome.route
    ...
    NavHost(
        ...
        composable("bookReader/{bookId}/{chapterNumber}") { backStackEntry ->
            val bookId = backStackEntry.arguments?.getString("bookId") ?: return@composable
